...
const SEED_VERSION_KEY = 'sem_seed_version';
const CURRENT_SEED_VERSION = '3'; // bump this whenever seed data shape changes

// ── Store helpers ─────────────────────────────────────────────────────────

/**
 * On startup, if the stored seed version doesn't match the current one,
 * wipe all localStorage keys so fresh seed data is loaded.
 */
const ensureSeedVersion = () => {
  const stored = localStorage.getItem(SEED_VERSION_KEY);
  if (stored !== CURRENT_SEED_VERSION) {
    [REQUESTS_KEY, EMPLOYEES_KEY, RATE_CONFIG_KEY, CITY_TIERS_KEY, COUNTRY_TIERS_KEY].forEach(
...
const computeDomesticSettlement = ({
  ...
  const conveyanceReimbursable = conveyanceActual;

  const totalReimbursable = perDiemTotal + hotelReimbursable + flightReimbursable + conveyanceReimbursable;
...
const computeIntlSettlement = ({
  ...
  const flightReimbursable = flightActual;
  const conveyanceReimbursable = conveyanceActual;
  const winterClothesReimbursable = winterClothesActual;

  const totalReimbursable = perDiemTotal + hotelReimbursable + flightReimbursable + conveyanceReimbursable + winterClothesReimbursable;
 ...
 * @param {string} [dto.draftId] - If submitting a draft, the draft ID.
...
export const createTravelPreApproval = async (dto) => {
  ...
  const baseRequest = {
    ...
    documents: {
      preApproval: {
        knoxApproval: dto.knoxApproval || null,
        travelInsurance: dto.travelInsurance || null,
        visa: dto.visa || null,
  ...
  let requestToReturn;
  if (dto.draftId) {
    const idx = requests.findIndex(r => r.id === dto.draftId);
    if (idx >= 0) {
      requests[idx] = { ...requests[idx], ...baseRequest, id: dto.draftId };
      requestToReturn = requests[idx];
    }
  }
  
  if (!requestToReturn) {
    requestToReturn = { ...baseRequest, id: generateId() };
    requests.push(requestToReturn);
  }
...
export const submitSettlement = async (id, dto) => {
  ...
  if (request.subtype === 'domestic') {
    settlement = computeDomesticSettlement({
      ...
      conveyanceActual: dto.conveyanceActual,
    });
    ...
    settlement = computeIntlSettlement({
      ...
      flightActual: dto.flightActual,
      conveyanceActual: dto.conveyanceActual,
      winterClothesActual: dto.winterClothesActual,
    });
  } else {
  ...
  requests[index] = {
    ...
    settlement: {
      ...
      conveyanceDetails: dto.conveyanceDetails,
    },
    documents: {
      ...request.documents,
...
export const createInternetRequest = async (dto) => {
  ...
  const baseRequest = {
    ghrId: String(dto.ghrId).trim(),
    type: 'internet-bill', // Match reference if they check for internet-bill
    subtype: null,
    stage: 'review',
...
export const saveDraftRequest = async (dto) => {
  const requests = loadRequests();
  const draftId = dto.draftId || generateId();
  ...
  const existingIdx = requests.findIndex(r => r.id === draftId);
  if (existingIdx >= 0) {
    requests[existingIdx] = { ...requests[existingIdx], ...baseRequest, id: draftId };
  } else {
    requests.push({ ...baseRequest, id: draftId });
  }

  persistRequests(requests);
  return requests.find(r => r.id === draftId);
};

/**
 * Extends an existing travel request by updating the end date.
 *
 * @param {string} id - Request ID.
 * @param {object} dto
 * @param {string} dto.revisedEndDate - New end date (ISO).
 * @param {Array} dto.approvalDocument - Uploaded revised approval document.
...
export const extendTrip = async (id, dto) => {
  ...
  if (request.type !== 'travel') {
    throw new Error('Only travel requests can be extended.');
  }
  ...
  requests[index] = {
    ...
    dates: {
      ...request.dates,
      endDate: dto.revisedEndDate,
    },
    documents: {
      ...request.documents,
      preApproval: {
        ...request.documents?.preApproval,
        knoxApproval: dto.approvalDocument,
      },
    },
  };
